...
1. **Generate skeletons**: `python3 scripts/build_knowledge_skeleton.py` —
   regenerates fact skeletons from `graphtr-out/graph.json` + `CLAUDE.md` + repo manifests
   (`Cargo.toml`/`package.json`/`requirements.txt`) into `graphtr-out/knowledge/<topic>.md`. Each
   skeleton has a `## Facts` section (auto-filled) and a `## Narrative` section with a
   `<!-- TODO: fill in -->` placeholder.

   Re-running this script overwrites all 7 files, including any narrative already written — only
   re-run it when the underlying facts have materially changed (e.g. after a real refactor), not
   as a routine step.
...
3. **Index into RAG**: `python3 scripts/index_knowledge.py` — mints a `rag_user_id`
   UUID into `graphtr-out/manifest.json` if absent, then for each topic: deletes any existing doc
   with a matching title, then uploads via `hoton-graphtr`'s `POST /api/documents`.
...
- Re-running `build_knowledge_skeleton.py` casually — it overwrites narrative you already wrote. Only re-run when facts genuinely changed.
